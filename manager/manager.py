...
class Manager(commands.Cog):
    ...
    @commands.command()
async def deliver(self, ctx, member: discord.Member, product: str, quantity: int, price: float, *, text: str):
    """Deliver a product to a member with custom message and vouch text."""
    
    # Split the input text into custom text and vouch text using a delimiter '|'
    try:
        # Ensure the delimiter '|' is in the text
        delimiter = '|'
        if delimiter not in text:
            raise ValueError("The text should be separated by `|`. Example format: `\"custom text\" | \"vouch text\"`")
        
        # Split the text into custom text and vouch text
        parts = text.split(delimiter, 1)
        if len(parts) != 2:
            raise ValueError("Please provide both custom text and vouch text separated by `|`. Example format: `\"custom text\" | \"vouch text\"`")
        
        # Extract custom text and vouch text
        custom_text = parts[0].strip().strip('"')
        vouch_text = parts[1].strip().strip('"')
